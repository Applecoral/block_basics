...
const episodes = Array.from({ length: 30 }, (_, i) => {
  ...
  return dynamic(() => import(`./episodes/chapter${chapter}/Episode${i + 1}`), {
    ssr: false,
    loading: () => <div className="text-[#00f2ff] animate-pulse">SYNCING DATA...</div>
  });
});
...
export default function GameController() {
  ...
  useEffect(() => {
    ...
    const saved = localStorage.getItem("on_chain_progress");
    if (saved) {
  ...
  useEffect(() => {
    if (gameStarted) {
      localStorage.setItem("on_chain_progress", currentStep.toString());
    }
  }, [currentStep, gameStarted]);

  const startNewGame = () => {
    localStorage.removeItem("on_chain_progress");
    setCurrentStep(0);
    setGameStarted(true);
  };
          ...
          <div className="flex flex-col gap-4">
            {hasSavedProgress ? (
              <>
                ...
                >
                  Resume Protocol (Day {parseInt(localStorage.getItem("on_chain_progress") || "0") + 1})
                </button>
                <button 
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[8px] text-white/40 animate-pulse">SAVING_TO_LOCAL_HOST</span>
              </div>
            </div>
          </div>

          <div className="flex justify-between items-center pointer-events-auto w-full max-w-2xl mx-auto">
            <button onClick={() => setCurrentStep(s => Math.max(0, s - 1))} className="text-white/20 hover:text-white text-[10px] uppercase p-2">Back</button>
            ...
            <button onClick={() => setCurrentStep(s => Math.min(29, s + 1))} className="text-white/20 hover:text-white text-[10px] uppercase p-2">Next</button>
          </div>
        </div>
      )}

      {/* Credit / Architect Link */}
      ...
      {gameStarted && (
        <div className="absolute inset-0 pointer-events-none flex flex-col justify-between p-6 z-10">
          ...
          <div className="flex justify-between items-center pointer-events-auto w-full">
            <button onClick={() => setCurrentStep(s => Math.max(0, s - 1))} className="text-white/20 hover:text-white text-[10px] uppercase">Back</button>
            <div className="flex gap-1">
              {Array.from({ length: 30 }).map((_, i) => (
                <div key={i} className={`h-1 w-2 ${i === currentStep ? 'bg-[#00f2ff] w-4' : i < currentStep ? 'bg-[#ff00ff]' : 'bg-white/10'}`}></div>
              ))}
            </div>
            <button onClick={() => setCurrentStep(s => Math.min(29, s + 1))} className="text-white/20 hover:text-white text-[10px] uppercase">Next</button>
          </div>
        </div>
      )}

      {/* Credit / Architect Link - Bottom Right */}
      <div className="absolute bottom-6 right-6 z-[60] flex items-center gap-2">
        <a 
          href="https://x.com/applecoraline" 
          target="_blank" 
